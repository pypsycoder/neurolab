#!/usr/bin/env python3
"""Persist one exact OpenAlex DOI metadata record for review-only evidence work."""
...
from neurolab.it_research import ItResearchQuery, ItResearchRun, lookup_openalex_doi, run_to_json
...
def main() -> None:
    ...
    parser.add_argument("--doi", required=True, help="Exact DOI identifier; URLs and free-text search are forbidden.")
    ...
    item = lookup_openalex_doi(arguments.doi, api_key=os.environ.get("OPENALEX_API_KEY"))
    run = ItResearchRun(
        ...
        audit=("targeted_openalex_doi_lookup", "public_metadata_only", "review_required"),
    )
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    metadata_path = OUTPUT_DIR / "latest-targeted-openalex-doi-record.json"
    ...
    persist_run(database_url, run, artifact_ref="runtime/it-research/latest-targeted-openalex-doi-record.json")
